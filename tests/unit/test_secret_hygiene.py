...
def test_group_vars_all_has_no_other_ansible_loadable_file(tracked):
    ...
    .json -- vault.yml (the real, gitignored secret) included. A
    tracked file that happens to match one of those extensions is
    loaded right alongside it, silently, which is exactly how
    vault.example.yml became a live fallback for the real vault (see
    docs/logbook/07-project-review.md, bug 45). Only main.yml -- the
    real, intentional vars file -- may have a loadable name here;
    anything else (vault.yml.example included) must not.
    """
